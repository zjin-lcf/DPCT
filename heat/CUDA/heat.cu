...
**          Ported to CUDA by Zheming Jin, June 2020
**
*/
...
__global__ void initial_value(const unsigned int n, const double dx,
                              const double length, double *u);
__global__ void zero(const unsigned int n, double *u);
__global__ void solve(const unsigned int n, const double alpha, const double dx,
                      const double dt, double *__restrict__ u,
                      double *__restrict__ u_tmp);
double solution(const double t, const double x, const double y,
                const double alpha, const double length);
double l2norm(const unsigned int n, const double *u, const int nsteps,
              const double dt, const double alpha, const double dx,
              const double length);
...
int main(int argc, char *argv[]) {
  ...
  double dx = length / (n + 1); // physical size of each cell (+1 as don't
                                // simulate boundaries as they are given)
  ...
  for (int t = 0; t < nsteps; ++t) {
    ...
    if (t % 2 == 0)
      solve<<< dim3(grid), dim3(block)>>>(n, alpha, dx, dt, u, u_tmp);
    else
      solve<<< dim3(grid), dim3(block)>>>(n, alpha, dx, dt, u_tmp, u);
  }
  ...
  std::cout
      ...
      << 1.0E-9 * 2.0 * n * n * nsteps * sizeof(double) /
             std::chrono::duration_cast<std::chrono::duration<double>>(toc -
                                                                       tic)
                 .count()
      << std::endl
      << LINE << std::endl;
...
__global__ void solve(const unsigned int n, const double alpha, const double dx,
  ...
  // Update the 5-point stencil, using boundary conditions on the edges of the
  // domain. Boundaries are zero because the MMS solution is zero there.
...
double l2norm(const unsigned int n, const double *u, const int nsteps,
  ...
  // Loop over the grid and compute difference of computed and known solutions
  // as an L2-norm
  double y = dx;
  for (int j = 0; j < n; ++j) {
    double x = dx;
